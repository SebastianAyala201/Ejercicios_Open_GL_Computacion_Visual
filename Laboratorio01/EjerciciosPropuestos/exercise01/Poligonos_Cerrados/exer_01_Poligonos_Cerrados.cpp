// Dibujando un poligono con GL_POLYGON
...
void pantalla(){
    ...
    glPushMatrix();
    glBegin(GL_POLYGON);
        for (ang = 0.0; ang < 2*M_PI; ang +=2*M_PI/7){
            x = radio * sin(ang);
            y = radio * cos(ang);
            glVertex2f(x,y);
        }
...
int main(int argc, char *argv[]){
    ...
    glutCreateWindow("HEPTAGONO con GL_POLYGON"); // Crea una ventana con el título especificado
    inicio(); 
